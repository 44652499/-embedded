...
class exception2  //*****只能这特定的divexception  exception2
{
template<class T>
...
public:
	...
	{}
	    //rbnode constructor
	rbnode(const T& val, rbnode<T> *l, rbnode<T> *r, rbnode<T> *p,ColorType c):value(val),left(l), right(r), parent(p), color(c)
	{}
...
public:
	...
	private:
		rbnode<T> *nodePtr;
		rbtree<T> *tree;   //++ --时当nodePtr为NIL时 用tree->root代替
	};
...
{
	//erase(pos.nodePtr->value);//nodePtr私有的
	erase(*pos);
}

template <class T> //切记删除的时候从end开始删除  因从start删除可能会造成后面迭代器保存的节点(被删除了)
void rbtree<T>::erase(iterator start, iterator end)
{
	iterator curr = end;
	--curr;
	if (start == begin() && end == rbtree<T>::end())  //节约效率
	{
		clear();
	}
	else				//从后面删除
