...
void  innit_undigraphic(undigraphic  * ptr_digrahic)
{
    ...
    for(i=0;i<ptr_digrahic->arcnum;i++)
    {
        ...
        ptr_new_node->ptr_s_next=ptr_digrahic->array[s].ptr_first;
        ptr_digrahic->array[s].ptr_first=ptr_new_node;

        ptr_new_node->ptr_e_next=ptr_digrahic->array[e].ptr_first;
        ptr_digrahic->array[e].ptr_first=ptr_new_node;
    }
}
//输出无向图
