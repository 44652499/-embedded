...
void  list_append(list* ptr_list,int num)
{
    ...
    for(i=0;i<num;i++)
    {
        ...
        if(ptr_list->ptr_head->next==NULL)
        {
            ...
            ptr_new->pre=ptr_list->ptr_head;    
        }
        else
        {
            ptr_list->ptr_cur->next=ptr_new;   
            ptr_new->pre=ptr_list->ptr_cur;    
        }  
        ptr_list->ptr_cur=ptr_new;   
        ptr_new=NULL;   
    }
    ...
    while(ptr_move!=NULL)
    {
        if(ptr_move->sid==dest_sid)
        {
            if(ptr_move==ptr_list->ptr_head->next)
            {
                ...
                ptr_move->next->pre=ptr_list->ptr_head;        
            }
            else
            {
...
//反向输出
void  list_reverse(list* ptr_list)
{
    node*  ptr_move=ptr_list->ptr_cur;
    while(ptr_move!=ptr_list->ptr_head)
    {
        printf("%d\t%s\n",ptr_move->sid,ptr_move->sname);
        ptr_move=ptr_move->pre;
    }
}
