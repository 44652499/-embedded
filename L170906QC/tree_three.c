...
{
    tree_node*  ptr_node;
    int  node_type;//1:表示栈第一次访问 2:表示栈第二次访问
    struct node * next;
}node;
...
//非递归后续遍历
void   print_last_tree(tree_node * root)
{
    stack  st;
    init_stack(&st);
    while(root!=NULL  ||  !is_stack_empty(&st))
    {
        ...
        {
            node* ptr_tmp=get_stack_top(&st);
            if(ptr_tmp->node_type==1)
            {
                ptr_tmp->node_type=2;
            }
            else
            {
                printf("%c\t",ptr_tmp->ptr_node->value);
                pop_stack(&st);
                continue;
            }
            root=ptr_tmp->ptr_node;
            root=root->rchild;
        }
    }
}
